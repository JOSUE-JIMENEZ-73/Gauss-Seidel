public class Algoritmos {
    ...
    public String metodoGaussSeidel(double[][] A, double[] b, double tolerancia) {
        ...
        salida += "Acomodando filas (Pivoteo Parcial)...\n";
        for (int k = 0; k < n; k++) {
            int filaMayor = k;
        ...
        do {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (xNuevo != 0) {
                    errorActual = redondear(Math.abs((xNuevo - x[i]) / xNuevo));
                }
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                comprobacion = redondear(comprobacion + redondear(matrizOriginal[i][j] * x[j]));
            }
            salida += "Fila " + (i + 1) + " calculada: " + comprobacion + " | Era: " + vectorOriginal[i] + "\n";
        }
